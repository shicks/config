-- TODO(sdh): reorganize this file, it's a huge mess.
...
globalkeys = awful.util.table.join(
    ...
    --     function ()
    --         awful.spawn(os.getenv("HOME") .. "/local/bin/suspend.sh") end)
    ...
    ,awful.key({ modkey, "Shift"  }, "Left",
	  ...
	  local idx = curidx - 1
	  if idx < 1 then idx = 9 end
	  local tag = client.focus.screen.tags[i]
	  client.focus:move_to_tag(client.focus.screen.tags[idx])
          awful.tag.viewprev()
        end
      end)
    ,awful.key({ modkey, "Shift"   }, "Right",
      function (c)
        if client.focus then
          local curidx = awful.tag.getidx()
	  local screen = awful.screen.focused()
	  local idx = math.fmod(curidx, 9) + 1
	  local tag = client.focus.screen.tags[i]
	  client.focus:move_to_tag(client.focus.screen.tags[idx])
          awful.tag.viewnext()
        end
    ...
    ,awful.key({ modkey, "Shift" }, "Return",
        function ()
            -- awful.spawn(terminal .. " -e " .. os.getenv("HOME") .. "/local/bin/tmux-start")
	    -- TODO(sdh): why doesn't this work?!?!?
            awful.spawn(terminal .. " -e " .. "tmux-start")
        end)
    ,awful.key({ modkey }, "b",
        function () awful.spawn(os.getenv("HOME") .. "/local/bin/rotate-bg") end)
...
-- SDH: Additional key bindings...
-- See http://www.holgerschurig.de/en/awesome-4.0-bindings/

local function globalkey(mod, key, func, desc)
...
local function clientkey(mod, key, func, desc)
   local key = awful.key(mod, key, func, desc)
   for k,v in pairs(key) do
...
-- Super+Ctrl+T: Toggle title bar on an individual window
...
client.connect_signal("request::titlebars", function(c)
    ...
    -- Hide the title bar if we're not floating
