-- NOTE: to debug:
-- tail -F `ls -l /proc/$(pidof awesome)/fd/2 | sed 's/.*-> //'`
...
-- Screenful (doesn't seem to actually work)
-- require("awful.remote") -- try again when we get home
-- require("screenful")
require("xrandr")
...
terminal = os.getenv("HOME") .. "/local/bin/urxvt"
editor = "quickemacs"
editor_cmd = terminal .. " -e " .. editor
...
vicious.register(myvol, vicious.widgets.volume, " $2:$1", 1, "Master")
...
globalkeys = awful.util.table.join(
    ...
    ,awful.key({ }, "XF86AudioRaiseVolume",
        function ()
            awful.util.spawn(os.getenv("HOME") .. "/local/bin/volume up")
        end)
    ,awful.key({ "Shift" }, "XF86AudioRaiseVolume",
        function ()
            awful.util.spawn(os.getenv("HOME") .. "/local/bin/volume up up up up")
        end)
    ,awful.key({ }, "XF86AudioLowerVolume",
        function ()
            awful.util.spawn(os.getenv("HOME") .. "/local/bin/volume down")
        end)
    ,awful.key({ "Shift" }, "XF86AudioLowerVolume",
        function ()
            awful.util.spawn(os.getenv("HOME") .. "/local/bin/volume down down down down")
        end)
