-- from http://awesome.naquadah.org/wiki/Using_Multiple_Screens 
...
-- the table)
local function iterate(t, filter, start)
    local count  = 0
    ...
    return function ()
        while count < length do
            local item = t[index]
            index = awful.util.cycle(#t, index + 1)
            count = count + 1
...
function switch()
  ...
  state.timer = capi.timer { timeout = 2 }
